...
public abstract class SIDAbstract extends PeripheralDevice {
	...
	SIDAbstract(int idSid, C64System memory) {
		super(idSid, memory);
	}
	...
	public abstract float getActualCutoff();
	abstract int filter(int in, int cycles, boolean lp, boolean hp, boolean bp);
	
	
	public int outputSample(int cycles) {
		...
		for(int i = 0; i < 3; ++i) {
			...
			case 0x50:	//tri+pulse
				...
				v.out &= v.triout & (((phase) <= /*v.pw*/0x3FF) ? (0xFFF) : (0));/*v.pulseout*/;
				break;
			case 0x60:	//saw+pulse
