...
import rsidplayer.CPU6510;
import rsidplayer.SIDFile;
import rsidplayer.SimpleSoundOutput;

public class RSIDPlayer {
	private static final int CIA1_TIMERA = 0xDC04;
	private static final int CIA1_TIMER_DEFAULT_NTSC = 0x4295;
	private static final int CIA1_TIMER_DEFAULT_PAL = 0x4025;

	private static final float C64_CLOCK_NTSC = 1022727.14f;
	private static final float C64_CLOCK_PAL = 985248.4f;
	...
	float clock_speed = C64_CLOCK_PAL;
	SIDFile sidFile;
	...
	public synchronized void initTune(int song) throws Exception {
		...
		// setup memory banks properly
				...
				c64.WriteMemory(C64System.PORT_6510, 0b00000101);
		...
		c64.WriteMemory(0xDC0D, 0b01111111);
		c64.WriteMemory(0xDC0D, 0b10000001);
		...
		c64.WriteMemory(0xDD0D, 0b01111111);
		...
		c64.WriteMemory(C64System.DDR_6510, 0b00101111);
		...
		if(sidFile.isPSID()) { 
			/* CIA timing */
			if(timingSource == CIA) {
				c64.WriteMemory(0xDC0D, 0b10000010);// enable cia 1a tmr interrupt
				c64.WriteMemory(0xD01A, 0x00);		// disable raster interrupt
			}else
			/* VIC Raster Int. timing */
			if(timingSource == VBI) {
				c64.WriteMemory(0xDC0D, 0b01111111);// disable cia interrupts
	...
	public void play() throws Exception {
		...
		final int sampleRate = 115200;
		byte[] buffer = new byte[sampleRate/20];
